...
/**
 * Query/resolution helpers for the two-sided patient facility-transfer
 ...
 * outside `modules/patients` because the decision steps are exposed under
 * both `modules/manager` and `modules/administrator` (the latter as the
 ...
 * "current state" of their transfer workflow, or `null` if they've never
 * had one (or their last one fully closed and the door's shut). Since a
 * request can't be created while one's already open, this row's `action`
 * alone tells you the stage: `TRANSFER_REQUESTED` (awaiting origin),
 * `TRANSFER_APPROVED_ORIGIN` (awaiting destination), or a closed one
 * (`TRANSFER_APPROVED_DESTINATION`/`TRANSFER_REJECTED`).
 */
export const getLatestTransferAction = async (
	core: TransferCore,
	patientId: string,
...
export const isTransferOpen = (action: TimelineAction): boolean =>
	action === TIMELINE_ACTION.TRANSFER_REQUESTED ||
	action === TIMELINE_ACTION.TRANSFER_APPROVED_ORIGIN;

/**
 * Batched version of `getLatestTransferAction` for a whole page of
 * candidate patient ids at once — the pending-transfers list can't just
 * filter `timeline` rows by action type, since a closed episode's
 * `TRANSFER_REQUESTED`/`TRANSFER_APPROVED_ORIGIN` rows never disappear;
 * each candidate has to be confirmed as still its patient's *current* row.
 * `limit: 1000` mirrors `patients/service.ts`'s flag-status batch lookup —
 * same "realistic volumes are nowhere near this" reasoning.
 */
export const getLatestTransferActionsByPatient = async (
	core: TransferCore,
	patientIds: string[],
 ...
 * original request row for whatever episode (open or closed) is current.
 * Needed to hydrate `reason`/`requested_by`/facility ids for rows further
 * along than the request itself, since there's no parent-request foreign
 * key linking a later decision row back to it (see module docstring).
 */
export const getLatestTransferRequestsByPatient = async (
	core: TransferCore,
	patientIds: string[],
): Promise<Map<string, TransferRow>> => {
 ...
 * an origin decision) or a later row from the same episode. Returns `null`
 * if `requestId` doesn't identify a request at all, or identifies one
 * that's been superseded by a newer request for the same patient (only
 * possible once the older one fully closed) — callers should treat both
 * cases as "not found," not try to distinguish them.
 */
export const resolveTransferRequest = async (
	core: TransferCore,
	requestId: string,
): Promise<{ request: TransferRow; latest: TransferRow } | null> => {
	const request = await core.timeline.one({
		where: {
			id: requestId,
			type: TIMELINE_TYPE.PATIENT,
			action: TIMELINE_ACTION.TRANSFER_REQUESTED,
	...
	const mostRecentRequest = await core.timeline.many({
		page: 1,
		limit: 1,
		where: {
			type: TIMELINE_TYPE.PATIENT,
	...
	const latest = await getLatestTransferAction(core, request.entity);
